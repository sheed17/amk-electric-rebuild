...
export interface Features8Item {
  icon: LucideIcon;
  title: string;
  description: string;
}

export interface Features8SupportCard {
  title: string;
  description: string;
}

export interface Features8Step {
  icon: LucideIcon;
  label: string;
  align: "left" | "right";
}
...
interface Features8Props {
  ...
  description?: string;
  items: Features8Item[];
  supportCard: Features8SupportCard;
  steps: Features8Step[];
  className?: string;
}
...
export function Features8({
  ...
  const [localItem, qualityItem, responsiveItem, communicationItem] = items;

  if (!localItem || !qualityItem || !responsiveItem || !communicationItem) {
    return null;
  }
  ...
  return (
    <section className={cn("bg-card/45 py-16 md:py-32", className)}>
      <div className="mx-auto max-w-3xl px-6 lg:max-w-6xl">
        <div className="mx-auto mb-12 max-w-3xl text-center md:mb-14">
          <h2 className="text-4xl font-serif leading-none text-primary md:text-5xl">
            {title}
          </h2>
        ...
        <div className="relative">
          <div className="relative z-10 grid grid-cols-6 gap-3">
            <Card className="relative col-span-full flex overflow-hidden border-border/70 bg-background/95 shadow-[0_28px_80px_-48px_rgba(13,24,46,0.28)] lg:col-span-2">
              <CardContent className="relative m-auto size-fit pt-6">
                <div className="relative flex h-24 w-56 items-center">
                  ...
                  <span className="mx-auto block w-fit text-4xl font-semibold tracking-tight text-primary">
                    San Jose
                  </span>
                </div>
                <div className="mt-6 flex items-center justify-center gap-2">
                  <localItem.icon className="size-4 text-gold" />
                  <h2 className="text-center text-3xl font-semibold text-primary">
                    {localItem.title}
                  </h2>
                </div>
                <p className="mt-3 max-w-xs text-center text-sm leading-7 text-muted-foreground">
                  {localItem.description}
                </p>
              </CardContent>
            </Card>

            <Card className="relative col-span-full overflow-hidden border-border/70 bg-background/95 shadow-[0_28px_80px_-48px_rgba(13,24,46,0.28)] sm:col-span-3 lg:col-span-2">
              <CardContent className="pt-6">
                <div className="relative mx-auto flex aspect-square size-32 rounded-full border border-border before:absolute before:-inset-2 before:rounded-full before:border before:border-border/60 dark:border-white/10 dark:before:border-white/5">
                  <svg
                ...
                <div className="relative z-10 mt-6 space-y-2 text-center">
                  <div className="flex items-center justify-center gap-2 text-gold">
                    <qualityItem.icon className="size-4" />
                    <span className="text-xs font-semibold uppercase tracking-[0.18em] text-gold">
                      Careful Execution
                    </span>
                  </div>
                  <h2 className="text-lg font-medium text-primary transition">
                    {qualityItem.title}
                  </h2>
                  <p className="text-foreground/80">{qualityItem.description}</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="relative col-span-full overflow-hidden border-border/70 bg-background/95 shadow-[0_28px_80px_-48px_rgba(13,24,46,0.28)] sm:col-span-3 lg:col-span-2">
              <CardContent className="flex h-full flex-col pt-6">
                <div className="relative z-10 mt-6 space-y-2">
                  ...
                  <h2 className="text-xl font-medium text-primary transition">
                    {supportCard.title}
                  </h2>
                ...
                <div className="mt-5 flex flex-wrap gap-2.5">
                  {coverageCities.map((city) => (
                    <span
                      key={city}
                      className="rounded-full border border-border/70 bg-background/82 px-3 py-1.5 text-xs font-medium text-primary/72"
                    >
                      {city}
                    </span>
                  ))}
            ...
            <Card className="relative col-span-full overflow-hidden border-border/70 bg-background/95 shadow-[0_28px_80px_-48px_rgba(13,24,46,0.28)] lg:col-span-3">
              <CardContent className="grid h-full gap-6 pt-6 sm:grid-cols-2">
                <div className="relative z-10 flex flex-col justify-between space-y-12 lg:space-y-6">
                  <div className="relative flex aspect-square size-12 rounded-full border border-border before:absolute before:-inset-2 before:rounded-full before:border before:border-border/60 dark:border-white/10 dark:before:border-white/5">
                    <responsiveItem.icon
                      className="m-auto size-5 text-gold"
                      strokeWidth={1.5}
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <h2 className="text-xl font-medium text-primary transition">
                      {responsiveItem.title}
                    </h2>
                    <p className="max-w-sm text-foreground/80">
                      {responsiveItem.description}
                    </p>
                  </div>
                </div>
                <div className="flex flex-col justify-center gap-4">
                  {responseMoments.map((moment) => (
                    <div
                      key={moment.title}
                      className="flex items-center gap-3 text-sm text-foreground/82"
                    >
                  ...
                  <div className="relative flex h-full flex-col justify-center space-y-6 py-6">
                    {steps.map((step, index) => (
                      ...
                      >
                        {step.align === "right" ? (
                          <>
                            <span className="block h-fit rounded border border-border/70 bg-background px-2 py-1 text-xs font-medium shadow-sm">
                              {step.label}
                            </span>
                            <div className="ring-background flex size-7 items-center justify-center rounded-full bg-gold text-accent-foreground ring-4">
                              <step.icon className="size-3.5" />
                            </div>
                          </>
                        ) : (
                          <>
                            <div className="ring-background flex size-8 items-center justify-center rounded-full bg-primary text-primary-foreground ring-4">
                        ...
                        <span className="sr-only">Communication step {index + 1}</span>
                      </div>
                    ))}
                  </div>
                </div>
